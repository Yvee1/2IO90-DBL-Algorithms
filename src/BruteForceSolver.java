...
class BruteForceSolver implements AlgorithmInterface {
     ...
     */
    public void solve(PackingProblem p) {
        setVariables(p);
    ...
    public void place(int i) {
        ...
        for (int x = 0; (x + r.getWidth()) <= maxWidth; x++) {
            r.setX(x);
            for (int y = 0; (y + r.getHeigth()) <= maxHeight; y++) {
                r.setY(y);
                if(isValid(r) && i < (rectangles.size() - 1)) {
                    place(i+1);
                } else if (isValid(r)) {
                    setArea();
    ...
    private boolean areClear(Rectangle r1, Rectangle r2) {
         ...
         */
        if ((r1.getY() + r1.getHeigtht()) < r2.getY() || (r2.getY() + r2.getHeigtht()) < r1.getY()) {
            return true;
        }
